...
describe('SendMoney Screen', () => {
  ...
  it('should transfer money from rich@payup.com to poor@payup.com and verify transaction in Home and History', () => {
    ...
    cy.get('input[placeholder="¿Para qué es esta transferencia?"]').type('Test transferencia');
    ...
    cy.get('.transactions-card').within(() => {
      ...
      cy.get('.description').contains('Test transferencia');
    ...
    cy.get('.transaction-item .description').contains('Test transferencia');
